...
def print_array(array, file):
    if array.ndim == 1:
        ...
        for dim1 in array:
            for dim2 in dim1:
                for dim3 in dim2:
                    file.write(', '.join(map(str, dim3)) + "\n")
    else:
        file.write("Unsupported shape.\n")

# def main():
#     path_to_hdf5 = "model_weights.h5"  # specify the path to your HDF5 file
...
def main():
    ...
    with open("weights.txt", "w") as file:  # Open file in write mode
        for key, value in weights.items():
            file.write(f"Key: {key}, Shape: {value.shape}\n")
            print_array(value, file)
            file.write("\n")  # Add an empty line between weights
