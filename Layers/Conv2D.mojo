...
fn get_subarray(img: Array3D, start_row: Int, start_col: Int, row_size: Int, col_size: Int, inout execution_time_get_subarray:Float32) -> Array3D:
    var start = now()
    ...
    var end = now()
    execution_time_get_subarray += end - start
    return subarray

fn multiply_and_sum(img: Array3D, filter: Array3D, inout execution_time_get_subarray: Float32) -> Float32:
    var start = now()
    var sum:Float32 = 0
    for i in range(img.dim0):
        for j in range(img.dim1):
...
fn Conv2D(img: Array3D, filter: Array3D, pad:Int, stride: Int, bias: List[Float32]) raises -> Array3D:
    var execution_time_get_subarray : Float32 = 0.0
    var execution_time_multiply_and_sum : Float32 = 0.0
    ...
    for i in range(total_filter):
        ...
        for j in range(n_h):
            for k in range(n_w):
                var subarray:Array3D = get_subarray(new_img,j*stride ,k*stride,f,f, execution_time_get_subarray)
                result.__setitem__(i,j,k, multiply_and_sum(subarray,new_filter, execution_time_get_subarray) + bias[i])
    ...
    # print("execution_time_multiply_and_sum:", execution_time_multiply_and_sum/1000000000)
    print("execution_time_get_subarray:", execution_time_get_subarray/1000000000)
    return ans