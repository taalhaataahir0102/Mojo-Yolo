...
from Datastructure.Array2D import Array4D, Array2D, Matrix
...
fn collect_weights(inout conv2D_weights:List[Array4D], inout dense_weights:List[Array2D], inout baises: List[List[Float32]]) raises:
    print("Collecting weights")
    var input = py.import_module("builtins").input
    Python.add_to_path("/home/talha/Desktop/mojo/tensor_yolo/parser")
    var mypython = Python.import_module("weights")
    var dict: PythonObject = mypython.read_hdf5("/home/talha/Desktop/mojo/tensor_yolo/parser/model_weights.h5")
    ...
    for i in range(len(shapes)):
        ...
        elif (layers[i].__contains__("dense") and layers[i].__contains__("kernel")) or (layers[i].__contains__("classifier_head") and layers[i].__contains__("kernel")) or (layers[i].__contains__("regressor_head") and layers[i].__contains__("kernel")):
    ...
    for i in range(len(layers)):
        var specific_weight: PythonObject = mypython.read2_hdf5("/home/talha/Desktop/mojo/tensor_yolo/parser/model_weights.h5", layers[i])

        if layers[i].__contains__("conv2d") and layers[i].__contains__("kernel"):
            for i in range(specific_weight.shape[0]):
                for j in range(specific_weight.shape[1]):
                    for k in range(specific_weight.shape[2]):
                        for l in range(specific_weight.shape[3]):
                            conv2D_weights[conv2D_index].__setitem__(i,j,k,l,specific_weight[i][j][k][l].to_float64().cast[DType.float32]())
                            total_trainable_wights+=1
            conv2D_index +=1
        ...
        if (layers[i].__contains__("dense") and layers[i].__contains__("kernel")) or (layers[i].__contains__("classifier_head") and layers[i].__contains__("kernel")) or (layers[i].__contains__("regressor_head") and layers[i].__contains__("kernel")):
            for i in range(specific_weight.shape[0]):
                for j in range(specific_weight.shape[1]):
                    dense_weights[dense_index].__setitem__(i,j,specific_weight[i][j].to_float64().cast[DType.float32]())
                    total_trainable_wights+=1
            dense_index+=1
        
        if layers[i].__contains__("bias"):
            for i in range(specific_weight.shape[0]):
                baises[bias_index].append(specific_weight[i].to_float64().cast[DType.float32]())
                total_trainable_wights+=1
            bias_index+=1
...
fn collect_weights_2(inout conv2D_weights:List[Tensor[DType.float32]], inout dense_weights:List[Tensor[DType.float32]], inout baises: List[Tensor[DType.float32]]) raises:
    print("Collecting weights")
    var input = py.import_module("builtins").input
    Python.add_to_path("/home/talha/Desktop/mojo/tensor_yolo/parser")
    var mypython = Python.import_module("weights")
    ...
    for i in range(len(shapes)):
        ...
        if layers[i].__contains__("conv2d") and layers[i].__contains__("kernel"):
            var new_filter = Tensor[DType.float32] (int_sizes[0], int_sizes[1], int_sizes[2], int_sizes[3])
            conv2D_weights.append(new_filter)
        elif layers[i].__contains__("bias"):
            var new_bias = Tensor[DType.float32] (int_sizes[0])
            baises.append(new_bias)
        elif (layers[i].__contains__("dense") and layers[i].__contains__("kernel")) or (layers[i].__contains__("classifier_head") and layers[i].__contains__("kernel")) or (layers[i].__contains__("regressor_head") and layers[i].__contains__("kernel")):
            var new_dense = Tensor[DType.float32] (int_sizes[0], int_sizes[1])
            dense_weights.append(new_dense)
    ...
    for i in range(len(layers)):
            ...
            for i in range(specific_weight.shape[0]):
                for j in range(specific_weight.shape[1]):
                    for k in range(specific_weight.shape[2]):
                        for l in range(specific_weight.shape[3]):
                            # conv2D_weights[conv2D_index].__setitem__(i,j,k,l,specific_weight[i][j][k][l].to_float64().cast[DType.float32]())
                            conv2D_weights[conv2D_index][Index(i,j,k,l)] = specific_weight[i][j][k][l].to_float64().cast[DType.float32]()
                            total_trainable_wights+=1
            conv2D_index +=1
        ...
        if (layers[i].__contains__("dense") and layers[i].__contains__("kernel")) or (layers[i].__contains__("classifier_head") and layers[i].__contains__("kernel")) or (layers[i].__contains__("regressor_head") and layers[i].__contains__("kernel")):
            for i in range(specific_weight.shape[0]):
                for j in range(specific_weight.shape[1]):
                    # dense_weights[dense_index].__setitem__(i,j,specific_weight[i][j].to_float64().cast[DType.float32]())
                    dense_weights[dense_index][Index(i,j)] = specific_weight[i][j].to_float64().cast[DType.float32]()
                    total_trainable_wights+=1
            dense_index+=1
        ...
        if layers[i].__contains__("bias"):
            for i in range(specific_weight.shape[0]):
                # baises[bias_index].append(specific_weight[i].to_float64().cast[DType.float32]())
                baises[bias_index][Index(i)] = specific_weight[i].to_float64().cast[DType.float32]()
                total_trainable_wights+=1
            bias_index+=1
